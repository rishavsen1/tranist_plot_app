...
def plot_max_aggregate(vis_df, DATE_START, DATE_END):
    all_route_id_dirs = vis_df.route_id_direction.unique()
    
    TIME_GRANULARITY = 60 #minutes
    ...
    for i, route_id_dir in enumerate(all_route_id_dirs):
        _df = vis_df[vis_df['route_id_direction'] == route_id_dir]
        if _df.empty:
            continue
